...
var _ = Describe("Plugin CEL Expression Evaluation", func() {
	Describe("CEL Expression Resolution", func() {
		DescribeTable("resolving CEL expressions with various functions",
			...
			Entry("simple cluster name",
				"self.global.greenhouse.clusterName",
				"obs-eu-de-1", false,
				map[string]any{
					"self": map[string]any{
			...
			Entry("simple organization name",
				"self.global.greenhouse.organizationName",
				"test-org", false,
				map[string]any{
					"self": map[string]any{
						"global": map[string]any{
			...
			Entry("upper case transformation",
				"self.global.greenhouse.clusterName.upperAscii()",
				"OBS-EU-DE-1", false,
				map[string]any{
					"self": map[string]any{
						"global": map[string]any{
			...
			Entry("lower case transformation",
				"self.global.greenhouse.clusterName.lowerAscii()",
				"obs-eu-de-1", false,
				map[string]any{
					"self": map[string]any{
						"global": map[string]any{
			...
			Entry("split and join with limit",
				"self.global.greenhouse.clusterName.split('-', 2).join('.')",
				"obs.eu-de-1", false,
				map[string]any{
					"self": map[string]any{
						"global": map[string]any{
			...
			Entry("split all parts and join",
				"self.global.greenhouse.clusterName.split('-').join('_')",
				"obs_eu_de_1", false,
				map[string]any{
					"self": map[string]any{
						"global": map[string]any{
			...
			Entry("https URL construction",
				"'https://' + self.global.greenhouse.clusterName + '.example.com'",
			...
			Entry("complex URL construction with split/join",
				"'ingress.' + self.global.greenhouse.clusterName.split('-', 2).join('.') + '.my.cloud.'",
			...
			Entry("conditional with ternary operator",
				"self.global.greenhouse.clusterName == 'obs-eu-de-1' ? 'production' : 'staging'",
				"production", false,
				map[string]any{
					"self": map[string]any{
						"global": map[string]any{
			...
			Entry("conditional with contains",
				"self.global.greenhouse.clusterName.contains('eu') ? 'europe' : 'other'",
				"europe", false,
				map[string]any{
					"self": map[string]any{
						"global": map[string]any{
			...
			Entry("join cluster names",
				"self.global.greenhouse.clusterNames.join(',')",
			...
			Entry("first cluster name by index",
				"self.global.greenhouse.clusterNames[0]",
				"obs-eu-de-1", false,
				map[string]any{
					"self": map[string]any{
						"global": map[string]any{
			...
			Entry("startsWith check",
				"self.global.greenhouse.clusterName.startsWith('obs') ? 'yes' : 'no'",
				"yes", false,
				map[string]any{
					"self": map[string]any{
						"global": map[string]any{
			...
			Entry("string size",
				"size(self.global.greenhouse.clusterName)",
				int64(11), false,
				map[string]any{
					"self": map[string]any{
						"global": map[string]any{
			...
			Entry("cluster metadata region",
				"self.global.greenhouse.metadata.region",
				"eu-de-1", false,
				map[string]any{
					"self": map[string]any{
						"global": map[string]any{
			...
			Entry("cluster metadata environment",
				"self.global.greenhouse.metadata.environment",
				"production", false,
				map[string]any{
					"self": map[string]any{
						"global": map[string]any{
			...
			Entry("metadata-based URL construction",
				"'https://api.' + self.global.greenhouse.metadata.region + '.example.com'",
			...
			Entry("conditional with metadata environment",
				"self.global.greenhouse.metadata.environment == 'production' ? 'prod-config' : 'dev-config'",
				"prod-config", false,
				map[string]any{
					"self": map[string]any{
						"global": map[string]any{
			...
			Entry("database connection string",
				"'postgres://user:pass@' + self.global.greenhouse.clusterName + '.db.example.com:5432/myapp'",
			...
			Entry("kubernetes service name",
				"self.global.greenhouse.organizationName + '-' + self.global.greenhouse.clusterName.split('-', 2).join('-') + '-svc'",
			...
			Entry("region-based domain - perses subdomain",
				"'perses.' + self.global.greenhouse.metadata.region + '.greenhouse.dev'",
			...
			Entry("vault secret path with region",
				"'vault+kvv2:///secrets/' + self.global.greenhouse.metadata.region + '/greenhouse-' + self.global.greenhouse.metadata.region + '/clientID'",
			...
			Entry("thanos query store URL with region",
				"'thanos-grpc.obs.' + self.global.greenhouse.metadata.region + '.cloud.sap:443'",
			...
			Entry("keystone password vault path with region",
				"'vault+kvv2:///secrets/' + self.global.greenhouse.metadata.region + '/thanos-greenhouse/keystone-user/thanos-greenhouse/password'",
			...
			Entry("prometheus username with region",
				"'prometheus-' + self.global.greenhouse.metadata.region + '-thanos'",
	...
	Describe("Complex Structures - Lists and Maps", func() {
		...
		It("should render map with dynamic region in nested values", func() {
			result, err := EvaluatePluginExpression(
				`{
					...
					'project_name': 'master',
					'region_name': self.global.greenhouse.metadata.region,
					'username': 'prometheus-' + self.global.greenhouse.metadata.region + '-thanos'
				}`,
				map[string]any{
					"self": map[string]any{
						"global": map[string]any{
			...
			Expect(ok).To(BeTrue(), "result should be a map[string]any")
		...
		It("should render list of multiple URLs with region", func() {
			result, err := EvaluatePluginExpression(
				`[
					'thanos-grpc.obs.' + self.global.greenhouse.metadata.region + '.cloud.sap:443',
					'thanos-grpc.mon.' + self.global.greenhouse.metadata.region + '.cloud.sap:443'
				]`,
				map[string]any{
					"self": map[string]any{
						"global": map[string]any{
		...
		It("should construct list from template with filter", func() {
			result, err := EvaluatePluginExpression(
				"self.global.greenhouse.metadata.region.startsWith('eu') ? ['eu-endpoint-1', 'eu-endpoint-2'] : ['us-endpoint-1', 'us-endpoint-2']",
				map[string]any{
					"self": map[string]any{
						"global": map[string]any{
							"greenhouse": map[string]any{
	...
	Describe("Edge Cases", func() {
		...
		It("should handle list size", func() {
			result, err := EvaluatePluginExpression(
				"size(self.global.greenhouse.clusterNames)",
				map[string]any{
					"self": map[string]any{
						"global": map[string]any{
							"greenhouse": map[string]any{
