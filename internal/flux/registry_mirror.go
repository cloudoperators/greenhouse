...
//	    subPath: "ghcr-mirror"
//
// An image like "ghcr.io/my-org/my-app:latest" becomes "europe.registry.com/ghcr-mirror/my-org/my-app:latest"
// The subPath is optional. If empty, images are mirrored directly without a path prefix.
...
func SetPluginConditionForRegistryMirrorError(plugin *greenhousev1alpha1.Plugin, err error) {
	condition := greenhousemetav1alpha1.TrueCondition(
		...
		fmt.Sprintf("Failed to read registry mirror configuration: %s", err.Error()),
	)
