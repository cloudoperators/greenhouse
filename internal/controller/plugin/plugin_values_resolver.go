...
func resolveOptionValuesFromValueRefs(ctx context.Context, c client.Client, log logr.Logger, plugin *greenhousev1alpha1.Plugin) ([]greenhousev1alpha1.PluginOptionValue, []string, error) {
	...
	for _, option := range optionValuesToBeResolved {
		resolvedValue, objectTrackers, err := resolveOptionValue(ctx, c, log, plugin, option, resolveKind, tracker)
		if err != nil {
			return nil, nil, err
		}
...
func resolveOptionValue(ctx context.Context, c client.Client, log logr.Logger, plugin *greenhousev1alpha1.Plugin, option greenhousev1alpha1.PluginOptionValue, defaultKind, tracker string) (*greenhousev1alpha1.PluginOptionValue, []string, error) {
	var value any
	...
	if option.ValueFrom.Ref.Name != "" {
		value, err = resolveByName(ctx, c, log, plugin, option, gvk, tracker)
		if err != nil {
			return nil, nil, err
		}
	...
	if option.ValueFrom.Ref.Selector != nil {
		...
		value, selectorTrackedObjects, err = resolveBySelector(ctx, c, log, plugin, option, gvk, tracker)
		if err != nil {
			return nil, nil, err
		}
	...
	if value != nil {
		...
		if err != nil {
			log.Error(err, "failed to marshal resolved value",
...
func resolveByName(ctx context.Context, c client.Client, log logr.Logger, plugin *greenhousev1alpha1.Plugin, option greenhousev1alpha1.PluginOptionValue, gvk schema.GroupVersionKind, tracker string) (any, error) {
	...
	if err != nil {
		if apierrors.IsNotFound(err) {
			log.Info("object does not exist, skipping value resolution...",
				"kind", gvk.Kind,
				"namespace", key.Namespace,
				"name", key.Name)
			return nil, nil
		}
		log.Error(err, "failed to get external value",
			"kind", gvk.Kind,
			"name", key.Name,
			"namespace", key.Namespace)
		return nil, err
	}
	value, err := evaluateExpression(log, uObject, option.ValueFrom.Ref.Expression, gvk, key)
	if err != nil {
		return nil, err
	}
	// add tracking information
	if err := annotateObjectWithTracking(ctx, c, gvk, key, tracker); err != nil {
		log.Error(err, "failed to annotate external object with tracking info, will retry on next reconciliation",
			"kind", gvk.Kind,
			"namespace", key.Namespace,
			"name", key.Name)
	}
...
func resolveBySelector(ctx context.Context, c client.Client, log logr.Logger, plugin *greenhousev1alpha1.Plugin, option greenhousev1alpha1.PluginOptionValue, gvk schema.GroupVersionKind, tracker string) (value any, trackedObjects []string, err error) {
	...
	value, trackedObjects, err = resolveMany(ctx, c, log, gvk, selector, plugin.GetNamespace(), option.ValueFrom.Ref.Expression, tracker)
	if err != nil {
		return nil, nil, err
	}
...
func evaluateExpression(log logr.Logger, uObject *unstructured.Unstructured, expression string, gvk schema.GroupVersionKind, key types.NamespacedName) (any, error) {
...
func resolveMany(ctx context.Context, c client.Client, log logr.Logger, gvk schema.GroupVersionKind, selector labels.Selector, namespace, expression, tracker string) (values any, trackedObjects []string, err error) {
	...
	if err := c.List(
		...
		log.Error(err, "failed to list external values",
			"kind", gvk.Kind,
			"labelSelector", selector)
		return nil, nil, err
	}
	values, trackedObjects, err = processResourceList(ctx, c, log, uObjectList.Items, gvk, expression, tracker)
	if err != nil {
		return nil, nil, err
	}
...
func processResourceList(ctx context.Context, c client.Client, log logr.Logger, items []unstructured.Unstructured, gvk schema.GroupVersionKind, expression, tracker string) (values []any, trackedObjects []string, err error) {
	for _, item := range items {
		// avoid self-referencing resources to prevent circular dependencies
		self := trackingID(gvk.Kind, item.GetName())
		if self == tracker {
			log.Info("skipping self-referencing resource to avoid circular dependency",
				"kind", gvk.Kind,
...
func removeUntrackedObjectAnnotations(ctx context.Context, c client.Client, log logr.Logger, plugin *greenhousev1alpha1.Plugin, currentTrackedObjects []string) error {
	...
	for _, untrackedObjectID := range untrackedObjects {
		if err := removeTrackingAnnotation(ctx, c, log, plugin.GetNamespace(), untrackedObjectID, tracker); err != nil {
			log.Error(err, "failed to remove tracking annotation from untracked object",
...
func removeTrackingAnnotation(ctx context.Context, c client.Client, log logr.Logger, namespace, objectID, tracker string) error {
	...
	return retry.RetryOnConflict(retry.DefaultRetry, func() error {
		...
		if err := c.Get(ctx, key, uObject); err != nil {
			if apierrors.IsNotFound(err) {
				...
				log.Info("untracked object not found, skipping cleanup",
					"kind", kind,
					"namespace", namespace,
					"name", name)
				return nil
		...
		case len(updatedTrackers) == 0:
			...
			log.Info("removed tracking annotation from resource",
				"kind", kind,
				"namespace", namespace,
				"name", name,
				"tracker", tracker)
		...
		case len(updatedTrackers) < len(trackers):
			...
			log.Info("removed tracker from resource",
				"kind", kind,
				"namespace", namespace,
				"name", name,
				"tracker", tracker)
...
func updateResourceWithAnnotation(ctx context.Context, c client.Client, gvk schema.GroupVersionKind, key types.NamespacedName, annotationUpdater func(map[string]string)) error {
	return retry.RetryOnConflict(retry.DefaultRetry, func() error {
		uObject := &unstructured.Unstructured{}
		uObject.SetGroupVersionKind(gvk)
