...
func (r *PluginPresetReconciler) resolvePluginOptionValuesForPreset(
	...
	if r.ExpressionEvaluationEnabled {
		...
		resolvedValues = convertToPluginOptionValues(preset.Spec.Plugin.OptionValues)
	}
...
func (r *PluginPresetReconciler) resolveReferencedPresetValues(
	...
	if !r.ExpressionEvaluationEnabled {
		return convertToPluginOptionValues(refPreset.Spec.Plugin.OptionValues)
	}
	...
	if err != nil {
		...
		log.Error(err, "Failed to resolve expressions in referenced PluginPreset, using raw values",
			"name", refPreset.Name)
		return convertToPluginOptionValues(refPresetWithOverrides.Spec.Plugin.OptionValues)
	}
