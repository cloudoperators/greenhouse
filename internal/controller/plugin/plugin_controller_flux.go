...
func (r *PluginReconciler) EnsureFluxCreated(ctx context.Context, plugin *greenhousev1alpha1.Plugin) (ctrl.Result, lifecycle.ReconcileResult, error) {
	...
	if pluginDefinitionSpec.HelmChart == nil {
		logger.Info("No HelmChart defined in PluginDefinition, skipping HelmRelease creation", "plugin", plugin.Name)
	...
	optionValues, err := r.prepareReleaseOptionValues(ctx, logger, plugin)
	if err != nil {
		plugin.SetCondition(greenhousemetav1alpha1.TrueCondition(
...
func (r *PluginReconciler) prepareReleaseOptionValues(ctx context.Context, logger logr.Logger, plugin *greenhousev1alpha1.Plugin) ([]greenhousev1alpha1.PluginOptionValue, error) {
	optionValues, err := resolvePlainOptionValues(ctx, r.Client, plugin.DeepCopy(), r.ExpressionEvaluationEnabled)
	if err != nil {
		return nil, err
	}
	if r.IntegrationEnabled {
		...
		optionValuesFromValueRefs, trackedObjects, err = resolveOptionValuesFromValueRefs(ctx, r.Client, logger, plugin.DeepCopy())
		if err != nil {
			logger.Error(err, "failed to resolve option values from value refs", "namespace", plugin.Namespace, "plugin", plugin.Name)
			return nil, err
		}
		...
		if err := removeUntrackedObjectAnnotations(ctx, r.Client, logger, plugin, trackedObjects); err != nil {
			logger.Error(err, "failed to remove untracked object annotations", "namespace", plugin.Namespace, "plugin", plugin.Name)
...
func (r *PluginReconciler) getValueReferences(plugin *greenhousev1alpha1.Plugin) []helmv2.ValuesReference {
...
func optionValuesToHelmReleaseValues(optionValues []greenhousev1alpha1.PluginOptionValue) ([]byte, error) {
...
func (r *PluginReconciler) ensureHelmRelease(
	...
	releaseValues, err := optionValuesToHelmReleaseValues(optionValues)
	if err != nil {
		log.FromContext(context.Background()).Error(err, "unable to create release values for plugin", "namespace", plugin.Namespace, "plugin", plugin.Name)
		return err
	}
	...
	builder := flux.NewHelmReleaseSpecBuilder().
		...
		WithValuesFrom(r.getValueReferences(plugin)).
...
// computeReadyCondition calculates the Ready condition for the Plugin based on the status of Flux Helm release
...
func (r *PluginReconciler) reconcileTrackingResources(ctx context.Context, plugin *greenhousev1alpha1.Plugin, oldChecksum, newChecksum string) {
	...
	for _, trackerID := range trackerIDs {
		if err := r.triggerReconcileForTracker(ctx, plugin, trackerID); err != nil {
			logger.Error(err, "failed to trigger reconciliation for tracking resource", "trackerID", trackerID)
		}
	}
}
...
func (r *PluginReconciler) triggerReconcileForTracker(ctx context.Context, plugin *greenhousev1alpha1.Plugin, trackerID string) error {
	...
	if err != nil {
		logger.Error(err, "invalid tracker ID format", "trackerID", trackerID)
		return err
	}
	...
	err = updateResourceWithAnnotation(ctx, r.Client, gvk, key, func(annotations map[string]string) {
		annotations[lifecycle.ReconcileAnnotation] = time.Now().Format(time.DateTime)
	})

	if err != nil {
		logger.Error(err, "failed to annotate tracking object with reconcile request",
			"kind", kind,
			"namespace", plugin.GetNamespace(),
			"name", name)
		return err
