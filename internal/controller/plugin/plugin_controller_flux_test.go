...
var _ = Describe("Flux Plugin Controller", Ordered, func() {
	...
	It("should create HelmRelease for Plugin", func() {
		...
		Eventually(func(g Gomega) {
			...
			g.Expect(release.GetAnnotations()).Should(HaveKeyWithValue(greenhouseapis.FluxResetRequestAnnotation, "foobar"), "HelmRelease should have the reset annotation updated to allow retry reset")
		...
		Eventually(func(g Gomega) {
			...
			g.Expect(release.GetAnnotations()).ShouldNot(HaveKey(greenhouseapis.FluxResetRequestAnnotation), "HelmRelease should have the reset annotation removed")
