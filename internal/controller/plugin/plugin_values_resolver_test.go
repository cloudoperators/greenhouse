...
var _ = Describe("PluginValuesResolver Helper Functions", func() {
	...
	Describe("filterExternalOptionValues", func() {
		It("should filter out option values with secrets", func() {
			optionValues := []greenhousev1alpha1.PluginOptionValue{
				...
				{
					Name:  "another-plain-value",
					Value: &apiextensionsv1.JSON{Raw: []byte(`"test2"`)},
				},
			}

			result := filterExternalOptionValues(optionValues)

			Expect(result).To(HaveLen(2))
			Expect(result[0].Name).To(Equal("plain-value"))
			Expect(result[1].Name).To(Equal("another-plain-value"))
		})

		It("should filter out option values with external references", func() {
			optionValues := []greenhousev1alpha1.PluginOptionValue{
				{
					Name:  "plain-value",
					Value: &apiextensionsv1.JSON{Raw: []byte(`"test"`)},
		...
		It("should return all values when none have external sources", func() {
			optionValues := []greenhousev1alpha1.PluginOptionValue{
				{
					Name:  "value1",
					Value: &apiextensionsv1.JSON{Raw: []byte(`"test1"`)},
				},
				{
					Name:  "value2",
					Value: &apiextensionsv1.JSON{Raw: []byte(`"test2"`)},
		...
		It("should return empty slice when all values are external", func() {
			optionValues := []greenhousev1alpha1.PluginOptionValue{
				{
					Name: "secret-value",
					ValueFrom: &greenhousev1alpha1.PluginValueFromSource{
