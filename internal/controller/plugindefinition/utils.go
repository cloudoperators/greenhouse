...
// enqueueOwnersForHelmRepoAndChart returns reconcile requests for all PluginDefinitions || ClusterPluginDefinitions
// that own the given HelmChart / HelmRepository, depending on the reconciler type.
func enqueueOwnersForHelmRepoAndChart(obj client.Object, ownerKind string) []ctrl.Request {
...
func (h *helmer) createUpdateHelmChart(ctx context.Context, helmRepo *sourcev1.HelmRepository) (*sourcev1.HelmChart, error) {
	...
	result, err := controllerutil.CreateOrUpdate(ctx, h.k8sClient, helmChart, func() error {
		...
		return controllerutil.SetOwnerReference(h.pluginDef, helmChart, h.k8sClient.Scheme())
	})
	if err != nil {
		return nil, err
	}
