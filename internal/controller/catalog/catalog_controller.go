...
func (r *CatalogReconciler) EnsureCreated(ctx context.Context, obj lifecycle.RuntimeObject) (ctrl.Result, lifecycle.ReconcileResult, error) {
	...
	for _, s := range catalog.Spec.Sources {
		...
		if shouldRequeue, err = sourcer.reconcileGitRepository(ctx); err != nil {
			...
			allErrors = append(allErrors, err.Error())
			sourcer.setInventoryErr(sourcev1.GitRepositoryKind, sourcer.getGitRepoName(), err.Error())
			continue
		}
		if shouldRequeue {
		...
		if externalArtifact, shouldRequeue, err = sourcer.reconcileArtifactGeneration(ctx); err != nil {
			...
			allErrors = append(allErrors, err.Error())
			sourcer.setInventoryErr(sourcev2.ArtifactGeneratorKind, sourcer.getGeneratorName(), err.Error())
			sourcer.setInventoryErr(sourcev1.ExternalArtifactKind, sourcer.getArtifactName(), err.Error())
			continue
		}
		if shouldRequeue {
			return ctrl.Result{RequeueAfter: 5 * time.Second}, lifecycle.Pending, nil
		}
		if shouldRequeue, err = sourcer.reconcileKustomization(ctx, externalArtifact); err != nil {
			...
			allErrors = append(allErrors, err.Error())
			sourcer.setInventoryErr(kustomizev1.KustomizationKind, sourcer.getKustomizationName(), err.Error())
			continue
		}
		if shouldRequeue {
			return ctrl.Result{RequeueAfter: 5 * time.Second}, lifecycle.Pending, nil
	...
	if len(allErrors) > 0 {
		errMsg := "not all catalog sources were reconciled successfully: " + strings.Join(allErrors, ", ")
	...
	catalog.SetTrueCondition(greenhousemetav1alpha1.ReadyCondition, greenhousev1alpha1.CatalogReadyReason, "all catalog sources reconciled successfully")
