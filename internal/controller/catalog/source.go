...
func (s *source) setInventoryErr(kind, name, msg string) {
	s.catalog.SetInventory(s.getSourceGroupHash(), kind, name, msg)
}
...
func (s *source) setInventory(kind, name string) {
	s.catalog.SetInventory(s.getSourceGroupHash(), kind, name, "")
}
...
func (s *source) reconcileGitRepository(ctx context.Context) (bool, error) {
	...
	requeue, err := s.isCatalogObjectReady(ctx, gitRepo)
	if err != nil {
		return requeue, err
	}
	s.setInventory(sourcev1.GitRepositoryKind, s.gitRepo.Name)
	return requeue, nil
}
...
func (s *source) reconcileArtifactGeneration(ctx context.Context) (extArtifact *sourcev1.ExternalArtifact, requeue bool, err error) {
	...
	requeue, err = s.isCatalogObjectReady(ctx, generator)
	if err != nil {
		return
	}
	if requeue {
	...
	requeue, err = s.isCatalogObjectReady(ctx, extArtifact)
	if err != nil {
		return
	}
	s.setInventory(sourcev2.ArtifactGeneratorKind, s.generator.Name)
	s.setInventory(sourcev1.ExternalArtifactKind, s.externalArtifact.Name)
	return
}

// reconcileKustomization - ensures the Kustomization resource is up to date
func (s *source) reconcileKustomization(ctx context.Context, extArtifact *sourcev1.ExternalArtifact) (requeue bool, err error) {
	...
	requeue, err = s.isCatalogObjectReady(ctx, kustomization)
	if err != nil {
		return
	}
	s.setInventory(kustomizev1.KustomizationKind, s.kustomize.Name)
	return
}

// isCatalogObjectReady - checks the Ready condition of a catalog object (GitRepository, ArtifactGenerator, ExternalArtifact, Kustomization)
// if not Ready, then the controller adds the Catalog object to requeue
func (s *source) isCatalogObjectReady(ctx context.Context, obj client.Object) (requeue bool, err error) {
	...
	case metav1.ConditionTrue:
		s.log.Info("Object is ready", "kind", kind, "namespace", key.Namespace, "name", key.Name)
		return
	case metav1.ConditionFalse:
		err = fmt.Errorf("object not ready: kind %s - %s/%s, reason: %s, message: %s", kind, key.Namespace, key.Name, readyCondition.Reason, readyCondition.Message)
		return
	default:
		s.log.Info("Object not ready yet, requeue...", "kind", kind, "namespace", key.Namespace, "name", key.Name, "reason", readyCondition.Reason, "message", readyCondition.Message)
		requeue = true
		return
	}
}
