...
`
			images := imagemirror.ExtractUniqueImages(manifests)
...
`
	...
	Describe("SplitImageRef", func() {
		It("should split fully qualified image ref", func() {
			reg, repo, tagOrDigest := imagemirror.SplitImageRef("ghcr.io/cloudoperators/greenhouse:main")
		...
		It("should default to docker.io for unqualified images", func() {
			reg, repo, tagOrDigest := imagemirror.SplitImageRef("nginx:latest")
		...
		It("should handle org/image format as docker.io", func() {
			reg, repo, tagOrDigest := imagemirror.SplitImageRef("myorg/myapp:v1.0")
			Expect(reg).To(Equal("docker.io"))
		...
		It("should handle digest references", func() {
			reg, repo, tagOrDigest := imagemirror.SplitImageRef("ghcr.io/cloudoperators/greenhouse@sha256:e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855")
			Expect(reg).To(Equal("ghcr.io"))
			Expect(repo).To(Equal("cloudoperators/greenhouse"))
		...
		It("should handle nested paths", func() {
			reg, repo, tagOrDigest := imagemirror.SplitImageRef("ghcr.io/org/team/project/app:v1.0")
			Expect(reg).To(Equal("ghcr.io"))
		...
		It("should default to :latest when tag is absent", func() {
			reg, repo, tagOrDigest := imagemirror.SplitImageRef("ghcr.io/cloudoperators/greenhouse")
			Expect(reg).To(Equal("ghcr.io"))
			Expect(repo).To(Equal("cloudoperators/greenhouse"))
			Expect(tagOrDigest).To(Equal(":latest"))
		})
