...
// ConfigMap and that its Plugin points its HelmRelease at it.
func FluxObjectLevelWorkloadIdentity(ctx context.Context, adminClient client.Client, env *shared.TestEnv, oidcClusterName, teamName string) {
	...
	Eventually(func(g Gomega) {
		...
		g.Expect(configMap.Data).To(HaveKeyWithValue("provider", "generic"))
		g.Expect(configMap.Data).To(HaveKeyWithValue("address", restConfig.Host))
		g.Expect(configMap.Data).To(HaveKeyWithValue("audiences", greenhouseapis.OIDCAudience))
		g.Expect(configMap.Data).To(HaveKeyWithValue("serviceAccountName", oidcClusterName))
		g.Expect(configMap.Data).To(HaveKeyWithValue("ca.crt", string(restConfig.CAData)))
