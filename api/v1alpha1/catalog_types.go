...
func (c *Catalog) SetInventory(hash, kind, name, errMsg string) {
	...
	if idx == -1 {
		c.Status.Inventory[hash] = append(c.Status.Inventory[hash], SourceStatus{Kind: kind, Name: name, ErrMsg: errMsg})
		return
	}
	c.Status.Inventory[hash][idx].ErrMsg = errMsg
}
